...
p dat u (xval($0)):(abs(($1))) w filledc below x tit '$k = 1$', \
  for [i=2:nlev] ''  u (xval($0)):(abs(column(i))) w filledc below x tit sprintf('$%d$', i), \
  '' u (xval($0)):(sqrt(column(1)**2 + column(2)**2 + column(3)**2 )) w filledc above x2 tit '$1,2,3$'
