...
set key at first 0.35, first 0.86 spacing 1.5 reverse Left samplen 1

p for [i=1:nlev] dat  u (xval($0)):(abs(column(i))) w linesp pointi 6 tit sprintf('$f_{%d}$', i), \
  '' u (xval($0)):(sqrt(column(1)**2 + column(2)**2 + column(3)**2 )) w linesp ps 0.5 pt 5 pointi 6 lc rgb color='red' tit '\tiny{$\sqrt{f_1^2 + f_2^2 + f_3^2}$}', \
...
unset grid
set key at 7.5,0.9
p dat_w u ($0+1):($1**2/180) w linesp pt 6 ps 0.5 tit sprintf('\small{$w_\kappa$}')
